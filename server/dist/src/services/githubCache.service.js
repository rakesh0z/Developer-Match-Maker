...
import { CACHE_TTL, cacheGet, cacheKey, cacheSet } from "./cache.service.js";
const githubHeaders = (token) => ({
    Accept: "application/vnd.github+json",
    ...(token ? { Authorization: `Bearer ${token}` } : {})
});
export const fetchIssuesCached = async (query, token) => {
...
export const getRepoMetadataCached = async (owner, repo, token) => {
    const key = cacheKey("github", "repo", owner, repo);
    const cached = await cacheGet(key);
    if (cached)
        return cached;
    const response = await axios.get(`https://api.github.com/repos/${owner}/${repo}`, { headers: githubHeaders(token) });
    const metadata = {
...
export const getRepoLanguageCached = async (repositoryUrl, token) => {
...
//# sourceMappingURL=githubCache.service.js.map